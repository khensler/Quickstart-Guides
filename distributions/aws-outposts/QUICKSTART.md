...
## Appendix: Legacy Manual Configuration

> **Note:** This section describes the manual configuration method used before the Dec 2024 AWS Console integration. Use this only if you need to configure iSCSI connections manually within the OS.

### Windows Manual iSCSI Configuration

1. **Get FlashArray iSCSI Portal IPs**:
   - Log into FlashArray GUI
   - Navigate to **Settings → Network**
   - Note the iSCSI interface IP addresses

2. **Configure Windows iSCSI Initiator**:
   ```powershell
   # Open iSCSI Initiator (run as Administrator)
   iscsicpl.exe
   ```
   - Go to **Configuration** tab → Copy the **Initiator Name** (IQN)
   - Add this IQN to the FlashArray host entry

3. **Connect to FlashArray**:
   - In iSCSI Initiator, go to **Targets** tab
   - Enter FlashArray portal IP under "Target"
   - Click **Quick Connect**
...
4. **Initialize Disk**:
   ...
   - **Initialize** and **Create New Simple Volume**

### Linux Manual iSCSI Configuration

```bash
# Install iSCSI tools
sudo dnf install -y iscsi-initiator-utils   # RHEL/Rocky
...
# Discover targets
sudo iscsiadm -m discovery -t st -p <flasharray-portal-ip>:3260

# Login to target
sudo iscsiadm -m node -T <target-iqn> -p <flasharray-portal-ip>:3260 --login
...
### Linux Manual NVMe-TCP Configuration

```bash
# Install NVMe tools
sudo dnf install -y nvme-cli               # RHEL/Rocky
sudo apt install -y nvme-cli               # Debian/Ubuntu

# Get host NQN
cat /etc/nvme/hostnqn

# Discover subsystems
sudo nvme discover -t tcp -a <flasharray-portal-ip> -s 4420

# Connect to subsystem
sudo nvme connect -t tcp -a <flasharray-portal-ip> -s 4420 -n <target-nqn>
