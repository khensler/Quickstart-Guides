---
layout: default
title: NFS on Oracle Linux - Quick Start Guide
---

# NFS on Oracle Linux - Quick Start Guide

This guide provides a streamlined path to configure NFS storage on Oracle Linux systems.
...
- Everpure FlashArray with:
  ...
  - Export path created (e.g., `/data/oracle`)
...
sudo mount -t nfs4 -o vers=4.1,nconnect=4,noatime,nodiratime \
    <NFS_SERVER_IP>:/data/oracle /mnt/pure-nfs
...
```bash
# Add fstab entry
echo '<NFS_SERVER_IP>:/data/oracle /mnt/pure-nfs nfs4 vers=4.1,hard,timeo=300,retrans=2,nconnect=4,noatime,nodiratime,_netdev 0 0' | sudo tee -a /etc/fstab
...
| Check kernel | `uname -r` (UEK vs RHCK) |
