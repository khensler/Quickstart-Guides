...
```bash
# Add to /etc/fstab
<NFS_SERVER_IP>:/data/rhel /mnt/pure-nfs nfs4 vers=4.1,hard,timeo=300,retrans=2,nconnect=4,noatime,nodiratime,_netdev 0 0
```
...
echo 'pure-nfs -fstype=nfs4,vers=4.1,hard,timeo=300,retrans=2,nconnect=4,noatime,nodiratime <NFS_SERVER_IP>:/data/rhel' | sudo tee /etc/auto.nfs
