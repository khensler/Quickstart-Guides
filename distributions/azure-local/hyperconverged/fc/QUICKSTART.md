---
layout: default
title: Azure Local Quick Start Guide - Hyperconverged FC
---

# Azure Local Quick Start Guide - Hyperconverged FC
...
Collect each node's FC **World Wide Port Names (WWPNs)** — you (or your storage admin) need these to register hosts and configure zoning on the FlashArray. Each node normally has **two or more** WWPNs (one per HBA port); record all of them.
...
> **Note:** These settings define the grace period for path failures. `NewPDORemovePeriod` determines how many seconds the Azure node OS waits for a failed path to recover. Holding the failover for 30 seconds lets MPIO complete a path-level failover without the Cluster Storage Service seeing the disk as "gone" and prematurely triggering a resource failover to a different node.

> **Note:** These are the recommended values for FlashArray integration, established through internal lab testing. Environments with highly specific latency requirements or complex fabric topologies may need additional testing to determine optimal timeout values for their workload.
...
- Create a **Host** object for each Azure Local node containing **all** of that node's WWPNs from Step 2.
...
# One host object per node. -Wwns takes an array — pass every WWPN the node has.
...
| Item | Required |
| --- | --- |
| LUNs created and connected to the **Host Group** (all nodes) | ✓ |
| Host entries created with **all** WWPNs per node, from Step 2 | ✓ |
...
> **Note:** MPIO settings from Step 3 do not take effect until the reboot, so `mpclaim -s -d` may report fewer paths than expected at this point. Re-run it after the reboot (Step 6) for the authoritative path count.
...
- Confirm the node's FlashArray host object contains **every** WWPN reported by `Get-InitiatorPort`, not just one.
- Confirm both fabrics are zoned, not just one.

**Local S2D disks appear alongside SAN LUNs**
- This is expected in a hyperconverged cluster. Filter by `BusType -eq 'Fibre Channel'` to target only FlashArray LUNs; never initialize or reformat S2D pool disks.
...
- Repeat Steps 7–10 for each additional FlashArray volume you want to expose as a CSV.
- Configure FlashArray snapshot and replication policies for the new CSVs.
- Review path counts and load-balance policy after any fabric or HBA change.
- Compare topologies before standardizing: [Disaggregated FC guide](../../disaggregated/fc/QUICKSTART.md) · [Hyperconverged iSCSI guide](../iscsi/QUICKSTART.md)

## Related Articles

- [Azure Local Quick Start Guide - Disaggregated FC](../../disaggregated/fc/QUICKSTART.md)
